...
def profile(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user = db.execute(
        text("SELECT id, full_name, email, phone, role, premium_expired_at FROM public.users WHERE id = :id"),
        {"id": current_user["sub"]}
    ).fetchone()
    if not user:
    ...
    return {"id": str(user.id), "name": user.full_name, "email": user.email, "phone": user.phone, "role": role, "premium_expired_at": str(user.premium_expired_at) if user.premium_expired_at else None}
